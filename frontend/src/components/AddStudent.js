...
const AddStudent = () => {
  ...
  const handleUpload = async () => {
        ...
        for (let sem = 1; sem <= 6; sem++) {
          ...
          for (let r = 0; r < semFilteredData.length; r++) {
            ...
            if (allData.some(element => (
              (element.rollNo + element.semester + element.year + element.stream.toUpperCase()) ===
              (semFilteredData[r].roll_no + semFilteredData[r].semester + semFilteredData[r].year + semFilteredData[r].stream.toUpperCase())
            ))) { continue; }
    ...
    for (let i = 0; i < allData.length; i++) {
      ...
      if (res.ok) {
        // alert('Marksheet data got uploaded!');
  ...
  const handleAdd = async () => {
    ...
    setLoading(false);
    if (data.error) {
      alert('Marksheet data already exist...!');
    }
    else if (data) {
      alert('Marksheet data got uploaded!');
    }
